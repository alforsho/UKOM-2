...
<div class="container py-5">
    <div class="card shadow-sm border-0 rounded-4 mx-auto" style="max-width: 800px;">
        ...
        <div class="card-body p-4">
            <div class="row g-4">
                ...
                <div class="col-12 mt-4">
                    <div class="alert alert-danger border-0 rounded-4 d-flex align-items-center shadow-sm">
                        <i class="fas fa-clock fa-2x me-3"></i>
                        <div>
                            <h6 class="mb-0 fw-bold text-danger">Terlambat {{ $transaksi->hari_terlambat }} Hari</h6>
                            <p class="mb-0">Denda Harus Dibayar: <strong>Rp {{ number_format($transaksi->total_denda, 0, ',', '.') }}</strong></p>
                        </div>
                    </div>
                </div>
                @endif
        ...
        <div class="card-footer bg-light border-0 p-3 text-center">
            <small class="text-muted">ID Transaksi Database: {{ $transaksi->id_transaksi }} | Update: {{ \Carbon\Carbon::parse($transaksi->updated_at)->format('d/m/Y H:i') }}</small>
        </div>
    </div>
</div>
@endsection